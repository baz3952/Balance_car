...
float Kalman_roll; 		//X最终倾斜角度
float Kalman_pitch; 		//Y最终倾斜角度
...
float dt      = 0.01;		//滤波算法计算周期，由定时器定时10ms
char  C_0     = 1;			//H矩阵值
...
void Kalman_Filter_X(float Accel,float Gyro) //卡尔曼函数		
{
	...
	//A(1,1) = 1,A(1,2) = -dt
	//A(2,1) = 0,A(2,2) = 1
	Kalman_roll += (Gyro - Q_bias) * dt; //状态方程,角度值等于上次最优角度加角速度减零漂后积分
	...
	Angle_err = Accel - Kalman_roll;	//Z(k)先验估计 计算角度偏差
	Kalman_roll += K_0 * Angle_err;	 //后验估计，给出最优估计值
